...
namespace UIGeiCar___Nairobi
{
    public partial class UIGeiCar : Form
    {
                ...
                while (clientSocket.Connected)
                {
                    ...
                    foreach (String msg in msgs)
                    {
                        ...
                        if (cmpt == 0)
                        {
                            ...
                            {
                                case "TOWSTATE":
                                    start_hook.BackgroundImage = Properties.Resources.ON;
        ...
        private void start_hook_Click(object sender, EventArgs e)
        {
            if (bConnected)
            {
                byte[] bytes = Encoding.ASCII.GetBytes("TOW:" + "request;");
                start_hook.BackgroundImage = Properties.Resources.wait;
                nwStream.Write(bytes, 0, bytes.Length);
            }
        }
